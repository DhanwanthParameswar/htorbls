...
<?php
$prefix = normalize_book_id($_GET["prefix"] ?? '');
$likePrefix = $prefix . '-%';

$result = db_select($mysqli, "SELECT bookId FROM booklist WHERE bookId LIKE ?", 's', [$likePrefix]);
...
if ($result && $result->num_rows > 0) {
    ...
    echo "<h4>Book ID Prefix: <b>" . h($prefix) . "</b></h4>";

    if (!empty($holes)) {
        echo "<h5><b>NOTICE:</b> Missing Book IDs (Holes):</h5>";
        echo "<ul>";
        foreach ($holes as $hole) {
            echo "<li>" . h("$prefix-$hole-1") . "</li>";
        }
        echo "</ul>";
    }

    $nextBookNumber = $maxBookNumber + 1;
    echo "<h4>Next available Book ID: <b>" . h("$prefix-$nextBookNumber-1") . "</b></h4>";
} else {
    echo "<h4>No books found for prefix <b>" . h($prefix) . "</b></h4>";
}
