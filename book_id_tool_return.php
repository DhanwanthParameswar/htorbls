...
function return_book(mysqli $mysqli, string $bookId, string $fineAmountPaid) {
  $row = db_fetch_one($mysqli,
    "SELECT patron_id, patronName, contactInfo, issueDate, dueDate FROM librarylog WHERE bookId = ?",
    's',
    [$bookId]
  );
  if (!$row) {
  ...
  if (!db_execute($mysqli,
    "INSERT INTO libraryarchive (patron_id, patronName, contactInfo, bookId, issueDate, dueDate, returnDate, fineAmountPaid) VALUES (?, ?, ?, ?, ?, ?, CURRENT_DATE, ?)",
    'isssssd',
    [$row['patron_id'], $row['patronName'], $row['contactInfo'], $bookId, $row['issueDate'], $row['dueDate'], $fineAmountPaid]
  )) {
    return false;
  }
