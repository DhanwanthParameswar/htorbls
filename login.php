...
<head>
	...
	<title><?= function_exists('demo_page_title') ? demo_page_title('Login') : 'HTOR BLS - Login' ?></title>
</head>
...
<?php if (function_exists("demo_render_banner")) demo_render_banner(); ?>
	  <div class="container text-center" style="width: 750px; background: #fff; border-radius: 10px; overflow: hidden; padding: 77px 55px 33px 55px; box-shadow: 0 5px 10px 0px rgba(0, 0, 0, 0.1); -moz-box-shadow: 0 5px 10px 0px rgba(0, 0, 0, 0.1); -webkit-box-shadow: 0 5px 10px 0px rgba(0, 0, 0, 0.1); -o-box-shadow: 0 5px 10px 0px rgba(0, 0, 0, 0.1); -ms-box-shadow: 0 5px 10px 0px rgba(0, 0, 0, 0.1);">
		...
		<form class="container" action="auth.php" method="post" style="width: 35rem">
			<h1 class="text-center display-4 mb-4"><?= function_exists('demo_page_title') ? htmlspecialchars(SITE_TITLE) : 'BLS - Login' ?></h1>
			<a href="help.php" class="btn btn-orange ml-4 mb-4">Help/Support</a>
			<br>
			<br>
			<?php if (defined('DEMO_MODE') && DEMO_MODE) { ?>
			<div class="alert alert-info" role="alert">You may use the demo account to sign in. Please note this is a sandboxed environment and any data saved will be erased after a day.</div>
			<?php } ?>
			<?php
			if (isset($_GET['error'])) {
			?>
		  ...
		  <div class="form-floating mb-3">
		    ...
		    <label for="inputUsername"><?= defined('DEMO_MODE') && DEMO_MODE ? 'Username | demo' : 'Username' ?></label>
		  </div>
		  <div class="form-floating mb-3">
		    <input type="password" name="password" class="form-control" id="inputPassword" placeholder="Password">
		    <label for="inputPassword"><?= defined('DEMO_MODE') && DEMO_MODE ? 'Password | DemoPass123' : 'Password' ?></label>
		  </div>
		  <br>
